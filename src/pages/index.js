...
import {POSTS_BY_CATEGORY} from "@/data/posts.js"
import FeaturedPosts from "@/components/FeaturedPosts"

const inter = Inter({ subsets: ['latin'] })

export default function Home({featuredPosts, topPosts ,aiPosts ,bigPosts,machinePosts}) {
  return (
    <>
      <Head>
...
/static/a381cbe123484c239b97a005d5ce4a00/5e493/image.jpg 1600w"
          ...
          >
            <div className="css-bh18vc">
              <div className="css-4p9r15">
                <div className="css-jw76fk">
                  <a className="css-18dwmz2" href="/category/ai/">
                    AI
                  </a>
                  <a className="css-1av0999" href="/category/ai/">
                    View More
                  </a>
                </div>
                <div className="css-1b24wpc" />
...
export async function getStaticProps() {
  try {
    const { data: postsByFeaturedData } = await client.query({
    ...
    const { data: postsByTopData } = await client.query({
      query: POSTS_BY_CATEGORY,
      variables: {
        categoryName : "TOP",
        first : 4
    ...
    const { data: postsByBigData } = await client.query({
      query: POSTS_BY_CATEGORY,
      variables: {
        categoryName : "BIGDATA",
        first : 6
      },
    });
    const { data: postsByMachineData } = await client.query({
      query: POSTS_BY_CATEGORY,
      variables: {
    ...
    return {
      props: {
        featuredPosts : postsByFeaturedData.posts.nodes,
        aiPosts : postsByAiData.posts.nodes,
        topPosts : postsByTopData.posts.nodes, 
        bigPosts : postsByBigData.posts.nodes,
        machinePosts : postsByMachineData.posts.nodes, 
      },
    };
  } catch (error) {
